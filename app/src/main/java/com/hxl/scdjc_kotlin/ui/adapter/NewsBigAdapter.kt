 ...
 * on 2018/4/4 星期三.
 */
class NewsBigAdapter(data: List<RspDto.Article>?, private val hasHeader: Boolean?) : BaseQuickAdapter<RspDto.Article, BaseViewHolder>(R.layout.item_news_big, data) {

    override fun convert(helper: BaseViewHolder, item: RspDto.Article) {
        val position: Int = if (hasHeader!!) {
        ...
        helper.setText(R.id.tv_title, data[position].title)
                ...
                .setGone(R.id.tv_has_news, data[position].today)
        Glide.with(mContext)
